...
void RestModel::setResourceName(QByteArray resourceName){
    ...
    Q_EMIT resourceNameChanged(resourceName);
}
...
void RestModel::p_serverError(){
    ...
    if(model.rowCount() == 0){ // only Q_EMIT noContentChanged when no data in the model, so ui reflects this correctly
...
void RestModel::resetStatus(bool loading, bool fetcing, bool noNetwork, bool noContent, bool noInternet){
    ...
    Q_EMIT fetchingChanged(fetcing);
    Q_EMIT noNetworkChanged(noNetwork);
    Q_EMIT noContentChanged(noContent);
    Q_EMIT noInternetChanged(noInternet);
}

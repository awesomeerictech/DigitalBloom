...
client_ = std::make_unique<Wt::Http::Client>();
//  client_->setMaximumResponseSize(0);
client_->setMaximumResponseSize(5000000);
//  client_->setTimeout(std::chrono::milliseconds{1000});

client_->done().connect(this, &DataUploader::requestDone);

qtSender_   = std::make_unique<WrapUpload>(this);
qtReceiver_ = std::make_unique<WrapUpload>(this);

// compile-time-checked connect (recommended)
bool ok = QObject::connect(
    qtSender_.get(), &WrapUpload::upload,
    qtReceiver_.get(), &WrapUpload::doUpload
    );

//Q_ASSERT(ok); // optional: catch connection errors in debug
...
void DataUploader::propagateUpload()
{
   // qtSender_->passUpload(const QJsonObject& payload);
}

void DataUploader::doUpload(const QJsonObject& payload)
{

}


// NOTE: You must replace these placeholders with the actual calls to your DataUploader instance
void DataUploader::newcategoryuploader(QVariantMap info) {
    qInfo() << "API Call: newcategory for:" << info.value("name").toString() << "ID:" << info.value("categories_id").toString();
    newcategory(info);
}
void DataUploader::newitemuploader(QVariantMap info) {
    qInfo() << "API Call: newitem for:" << info.value("name").toString() << "ActionDue: New";
    newitem(info);
}
void DataUploader::updateitemuploader(QVariantMap info) {
    qInfo() << "API Call: updateitem for:" << info.value("name").toString() << "ActionDue: Update";

   // updateitem(info);
    QJsonDocument userdoc(QJsonObject::fromVariantMap(info));

    qInfo() << "The original JSON is: " << qPrintable(userdoc.toJson()) << Qt::endl;
}
        ...
        if (currentActionDue == "new") {
            ...
            if (!uploadedCategories.contains(currentCategory)) {
                ...
                if (categoryLookup.contains(currentCategory)) {
                    ...
                    newcategoryuploader(categoryInfo); // Placeholder call
            ...
            newitemuploader(itemInfo); // Placeholder call
            newCount++;
            
        } else if (currentActionDue == "update") {

            itemInfo["itemsid"] = extractedData["itemsid"].toArray().at(i).toString();
            // Update Item Logic (no category check needed)
            updateitemuploader(itemInfo); // Placeholder call
            updateCount++;
...
void DataUploader::requestDone(Wt::AsioWrapper::error_code ec, const Wt::Http::Message &msg)
{
    if (!ec) {
         Wt::WStringStream ss;
        ss << "Status code " << msg.status() << "\n\n" << msg.body();
        qInfo() <<"Status Code is: " << msg.status() << Qt::endl;
        qInfo() << "Message Body is: " << QString::fromStdString(msg.body()) << Qt::endl;
        qInfo() << "Results are: " << QString::fromStdString(Wt::utf8(ss.str()).toUTF8()) << Qt::endl;
        if(msg.status()==200) {

            myrestdata_.emit(msg.body());
            QString themessage = QString("The record was added successfully");
            globalBackendHandler.triggerDialogToast(themessage);
            ...
            QString themessage = QString("The server responded with an error: ");
            themessage.append(QString::fromStdString(msg.body()));
            globalBackendHandler.triggerDialogError(themessage);
        ...
        qInfo() << "Errors are: " << QString::fromStdString(Wt::utf8("{1}").arg(ec.message()).toUTF8()) << Qt::endl;
        networkErrormyrest_.emit(Wt::utf8("{1}").arg(ec.message()).toUTF8());
        QString themessage = QString("A network error occurred: ");
        themessage.append(QString::fromStdString(msg.body()));
        globalBackendHandler.triggerDialogError(themessage);
...
void DataUploader::requestDoneWithin(Wt::AsioWrapper::error_code ec, const Wt::Http::Message &msg)
{
    if (!ec) {
...
void DataUploader::mybodyDataReceived(std::string bodydata) {


    // qInfo() << "Message Body mybodyDataReceived is: " << QString::fromStdString(bodydata) << Qt::endl;
    ...
    if(mypayload->parseRequest(QString::fromStdString(combimsg))) {
        ...
        // qInfo() << "Json is well constructed. " << Qt::endl;
        // qInfo() << "Message Body combimsg is: " << QString::fromStdString(combimsg) << Qt::endl;
        ...
        //qInfo() << "Json is being constructed! " << Qt::endl;
...
std::string DataUploader::fromIstream(std::istream &stream) {
...
void DataUploader::starttimer2() {

    return;

}

void DataUploader::starttimer3() {

    return;

}

void DataUploader::starttimer4() {

    return;

}

std::wstring DataUploader::myurldue(AppLinks links) {
...
std::wstring DataUploader::myroledue(AppRoles roles) {
...
void  DataUploader::uservalidity(QVariantMap info) {
...
void  DataUploader::categoryexistence(QVariantMap info) {
...
void  DataUploader::itemexistence(QVariantMap info) {
...
void DataUploader::loginuser(QVariantMap info) {
...

{
   "token": "mytoken"
    ...
    //qInfo() << "utc datedue is: " << datedue << Qt::endl;
    ...
    //qInfo() << "new userdoc is: " << qPrintable(userdoc.toJson(QJsonDocument::JsonFormat::Indented)) << Qt::endl;
    ...
    //qInfo() << "jwt payload is: " << jwt << Qt::endl;
...
void DataUploader::registeruser(QVariantMap info) {
    combimsg ="";
    //client_->abort();
    //std::string jsondata = R"( )";
...
void DataUploader::retrievepassword(QVariantMap info) {
...
void DataUploader::userdata(QVariantMap info) {
...
void DataUploader::usersubscription(QVariantMap info) {
...

{
   ...
   "data": {
    ...
    mypayload->modifyJsonValue(userdoc, "data.created_at", datedue);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for new category is: " << qPrintable(usagejson) << Qt::endl;
...
void DataUploader::deletecategory(QVariantMap info) {
...

{
   ...
   "data": {
    ...
    mypayload->modifyJsonValue(userdoc, "data.username", username);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for delete category is: " << qPrintable(usagejson) << Qt::endl;
...

{
   ...
   "data": {
    ...
    mypayload->modifyJsonValue(userdoc, "data.created_at", datedue);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for new item is: " << usagejson << Qt::endl;
...

{
   ...
   "data": {
    ...
    QJsonDocument myuserdoc(QJsonObject::fromVariantMap(info));

    qInfo() << "This is the payload for update: " << qPrintable(myuserdoc.toJson()) << Qt::endl;
    ...
    //mypayload->modifyJsonValue(userdoc, "data.created_at", datedue);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for update item is: " << qPrintable(usagejson) << Qt::endl;
...
void DataUploader::queryitems(QVariantMap info) {
...

{
  ...
  "request_id": "c8e42836-b04b-40dc-81da-919bbd00d5f8"
    ...
    mypayload->modifyJsonValue(userdoc, "data.username", username);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for query items is: " << qPrintable(usagejson) << Qt::endl;
    std::string myapiurl = QString::fromStdWString(myurldue(APIURL)).toStdString();
    Wt::Http::Message message;
    client_->setSslCertificateVerificationEnabled(false);
    message.addBodyText(usagejson.toStdString());
...
void DataUploader::userbyid(QVariantMap info) {
...

{
  ...
  "data": {
    ...
    //qInfo() << "standardized is: " << standardized << Qt::endl;
    //qInfo() << "standardizedavatar is: " << standardizedavatar << Qt::endl;
    ...
    mypayload->modifyJsonValue(userdoc, "data.username", username);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for userbyid is: " << qPrintable(usagejson) << Qt::endl;
...
void DataUploader::deleteitem(QVariantMap info) {
...

{
   ...
   "data": {
    ...
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for delete item is: " << qPrintable(usagejson) << Qt::endl;
...
void DataUploader::newrequest(QVariantMap info) {
...

{
  ...
  "data": {
    ...
    mypayload->modifyJsonValue(userdoc, "data.created_at", datedue);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for new request is: " << usagejson << Qt::endl;
...
void DataUploader::updaterequest(QVariantMap info) {
...
void DataUploader::deleterequest(QVariantMap info) {
...

{
  ...
  "request_id": "c8e42836-b04b-40dc-81da-919bbd00d5f8"
    ...
    mypayload->modifyJsonValue(userdoc, "data.username", username);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for delete orders is: " << qPrintable(usagejson) << Qt::endl;
...
void DataUploader::queryrequest(QVariantMap info) {
...

{
   ...
   "request_id": "c8e42836-b04b-40dc-81da-919bbd00d5f8"
    ...
    mypayload->modifyJsonValue(userdoc, "data.username", username);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for query orders is: " << qPrintable(usagejson) << Qt::endl;
    std::string myapiurl = QString::fromStdWString(myurldue(APIURL)).toStdString();
    Wt::Http::Message message;
    client_->setSslCertificateVerificationEnabled(false);
    message.addBodyText(usagejson.toStdString());
...
void DataUploader::deleteitemcategory(QVariantMap info) {
...
void DataUploader::fetchspecific(QVariantMap info) {
...
void DataUploader::passwordupdate(QVariantMap info) {
...

{
   ...
   "query": {
    ...
    mypayload->modifyJsonValue(userdoc, "data.created_at", datedue);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for password update is: " << qPrintable(usagejson) << Qt::endl;
...
void DataUploader::phonenumberupdate(QVariantMap info) {
...

{
   ...
   "query": {
    ...
    mypayload->modifyJsonValue(userdoc, "data.created_at", datedue);
    const QString usagejson = userdoc.toJson(QJsonDocument::JsonFormat::Indented);
    //qInfo()<<"Usage Json for phone number update is: " << qPrintable(usagejson) << Qt::endl;
...
void DataUploader::logout(QVariantMap info) {
...
void  DataUploader::getuseritems(QString user) {

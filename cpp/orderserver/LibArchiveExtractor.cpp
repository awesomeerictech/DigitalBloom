...
ArchiveMetadata LibArchiveExtractor::findMetadataInArchive(const QString &sourceFilePath) {
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        isMetadata |= readAndSplit(metadata.actionDues, foundActionDue, "actiondue.txt");
        isMetadata |= readAndSplit(metadata.idsIds, foundIdsId, "ids_id.txt");
        ...
        isMetadata |= readAndSplit(metadata.itemsIds, founditemsIds, "itemsids.txt");


        // If we found all *required* files, we can break early.
        if (foundUsername && foundCategory && foundItems && foundPrice && foundSizeOrQuantity && foundManufacturer && foundDescription && foundActionDue && foundIdsId) {
            break;
        }
...
QJsonObject LibArchiveExtractor::extractAndProcessTarXzFile(const QString &sourceFilePath,
    ...
    if (metadata.usernames.isEmpty() || metadata.categories.isEmpty() || metadata.items.isEmpty() ||
        ...
        metadata.actionDues.isEmpty() || metadata.idsIds.isEmpty()) {

        qWarning() << "Extraction failed: One or more required metadata files are missing or empty.";

        QString themessage = QString("Failed: One or more required metadata files are missing or empty.");
        globalBackendHandler.triggerDialogError(themessage);

        return QJsonObject();
    }

    if (metadata.itemsIds.isEmpty()) {
    ...
    const QStringList actionDuesList = metadata.actionDues;
    const QStringList idsIdsList = metadata.idsIds;
    QStringList itemsIdsList = QStringList();
    ...
    const QString primaryActionDue = actionDuesList.first();
    const QString primaryIdsId = idsIdsList.first();
    QString primaryitemsIds = QString();
    ...
    dataObject["ids_id"] = primaryIdsId;
    ...
    dataObject["actiondue"] = primaryActionDue;
    ...
    if(update) {
        ...
        itemsIdsList = metadata.itemsIds;
        primaryitemsIds = itemsIdsList.first();
        dataObject["items_id"] = primaryitemsIds;
        qInfo() << "we are updating item" << Qt::endl;
        qInfo() << "logging items_id from extractor " << dataObject.toVariantMap().value("items_id").toString()<< Qt::endl;
        ...
        qInfo() << "we are not updating item" << Qt::endl;
    }
    ...
    QJsonArray itemsIdArray, idsIdArray, uuidArray, mediaFileArray, nameArray, categoryArray,
        ...
        manufacturerArray, priceArray, descriptionArray, createdAtArray, actionDueArray;
    ...
    while (r == ARCHIVE_OK) {
        ...
        // --- FILE INDEXING LOGIC (1-based index from file name) ---
        QFileInfo originalFileInfo(originalFileName);
        QString fileNameWithoutExtension = originalFileInfo.baseName();

        QRegularExpression indexRegex("^\\d+");
        QRegularExpressionMatch match = indexRegex.match(fileNameWithoutExtension);

        int fileIndex = -1;

        if (match.hasMatch()) {
            QString indexStr = match.captured(0);
            bool ok;
            fileIndex = indexStr.toInt(&ok);
            if (!ok || fileIndex < 1) {
                fileIndex = -1;
            }
        }
        ...
        if (fileIndex == -1) {
            qWarning() << "Skipping file: Name does not start with a valid 1-based index (e.g., 001, 1):" << originalFileName;
            archive_read_data_skip(a);
            continue;
        }

        // Calculate 0-based list index
        const int listIndex = fileIndex - 1;

        // Check bounds against the shortest critical list (items list)
        if (listIndex >= itemsList.count()) {
            qWarning() << "Skipping file:" << originalFileName << ". Metadata index" << listIndex + 1 << "out of bounds (max entries:" << itemsList.count() << ").";
            archive_read_data_skip(a);
            continue;
        }

        // --- END FILE INDEXING LOGIC ---
        ...
        // Fetch current indexed values using listIndex (modulo retained for safety)
        const int index = listIndex;
        const QString currentUsername = usernamesList.at(index % usernamesList.count());
        const QString currentCategory = categoriesList.at(index % categoriesList.count());
        const QString currentItemName = itemsList.at(index % itemsList.count());
        const QString currentPrice = pricesList.at(index % pricesList.count());
        const QString currentSizeOrQuantity = sizesOrQuantitiesList.at(index % sizesOrQuantitiesList.count());
        const QString currentManufacturer = manufacturersList.at(index % manufacturersList.count());
        const QString currentDescription = descriptionsList.at(index % descriptionsList.count());
        const QString currentStandardized = standardizedList.at(index % standardizedList.count());
        const QString currentStandardizedAvatar = standardizedAvatarsList.at(index % standardizedAvatarsList.count());
        const QString currentActionDue = actionDuesList.at(index % actionDuesList.count());
        const QString currentIdsId = idsIdsList.at(index % idsIdsList.count());
        QString currentitemsIds = QString();
        ...
        idsIdArray.append(currentIdsId);
        ...
        usernameArray.append(currentUsername); // Correctly using indexed value
        ...
        actionDueArray.append(currentActionDue);

        createdAtArray.append(QDateTime::currentDateTimeUtc().toString(Qt::ISODate));

        if(update) {

            currentitemsIds = itemsIdsList.at(index % itemsIdsList.count());
            itemsIdArray.append(currentitemsIds);
        }
        ...
        QJsonDocument doc(itemsIdArray);
        QByteArray jsonData = doc.toJson(QJsonDocument::Indented); // Use Indented for pretty printing
        QString jsonString = QString::fromUtf8(jsonData);
        qInfo() << "itemsIdArray content after append:\n" << jsonString;

        if (filesExtractedCount == 0) {
            if(update) {

                dataObject["mediafile"] = newFileName;
                dataObject["itemsid"] = primaryitemsIds;
        ...
        if (!addedCategories.contains(currentCategory)) {
            ...
            categoryUsernameArray.append(currentUsername); // FIX: Now using indexed value
            categoryCreatedAtArray.append(QDateTime::currentDateTimeUtc().toString(Qt::ISODate));
            categoryIdsIdArray.append(currentIdsId);
    ...
    qInfo() << "logging items_id from extractor assemble " << dataObject.toVariantMap().value("items_id").toString()<< Qt::endl;

    QJsonDocument doc(itemsIdArray);
    QByteArray jsonData = doc.toJson(QJsonDocument::Indented); // Use Indented for pretty printing
    QString jsonString = QString::fromUtf8(jsonData);
    qInfo() << "is itemsIdArray still appended?:\n" << jsonString;

    if(update) {

        extractedItemsData["itemsid"] = itemsIdArray;
        qInfo() << "update value is relevant! " << Qt::endl;
    }
    else {

    extractedItemsData["items_id"] = itemsIdArray;
     qInfo() << "update value is not relevant! " << Qt::endl;

    }

    qInfo() << "logging itemsid from extractor assemble final " << extractedItemsData.toVariantMap().value("itemsid").toString()<< Qt::endl;
    ...
    extractedItemsData["actiondue"] = actionDueArray;
    ...
    // Placeholder for nested 'query' object (the outer object)
    ...
    queryOuter["criteria"] = queryInner;
    ...
    qInfo() << "Extraction and JSON generation complete. Processed" << filesExtractedCount << "media files.";

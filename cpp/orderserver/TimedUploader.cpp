    ...
    : QObject(parent),uploader_(uploader), mydatanet_(mydatanet)
{
    ...
    QObject::connect(
        &m_startitemdelayTimer,
        &QTimer::timeout,
        this,
        &TimedUploader::startafterdelay
        );

    m_uploadTimer.setInterval(200); // 200 ms = 1 second delay between calls
    m_startitemdelayTimer.setInterval(100);
    m_startitemdelayTimer.setSingleShot(true);
}
...
void TimedUploader::startafterdelay() {
    qInfo() << "We are delaying a little to allow for database transaction...";
    processNextItem();
...
void TimedUploader::processNextItem()
{
    ...
    if (m_currentPhase == CategoryPhase) {
            ...
            if (m_currentIndex < m_categoryCount) {
                ...
                if (m_itemCount > 0) {
                   // processNextItem(); // Start the Item Phase without delay
                    m_startitemdelayTimer.start(); // delay for database transaction
                } else {
        ...
        // QString mymessage = "Upload session finished. Total processed: Categories:" + QString::number(m_categoriesUploaded) +
  //                   ", Items (New:" + QString::number(m_newCount) +
  //                   ", Update:" + QString::number(m_updateCount) +
  //                   ", Skipped:" + QString::number(m_skippedCount) + ").";
					
		QString themessage = QString("Upload session finished. Total processed: Categories:%1, Items (New:%2, Update:%3, Skipped:%4).")
                        .arg(m_categoriesUploaded)
                        .arg(m_newCount)
                        .arg(m_updateCount)
                        .arg(m_skippedCount);
					
		 globalBackendHandler.triggerInfoDialog(themessage);
        return;
    }
}
...
void TimedUploader::processItemPhase() {
    ...
    if (currentActionDue == "new") {
        ...
        itemInfo["itemsid"] = m_extractedData["itemsid"].toArray().at(i).toString();
        updateitem(itemInfo);
        m_updateCount++;

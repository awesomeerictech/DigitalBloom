...
AndroidService::AndroidService(QObject *parent) : QObject(parent)
{
    ...
    m_notificationManager->setNotification("DigitalBloom starting", QDateTime::currentDateTime().toString(),0);
...
{
    m_notificationManager = NotificationManager::getInstance(); // DEBUG
    m_notificationManager->setNotification("DigitalBloom Running", QDateTime::currentDateTime().toString(),1);

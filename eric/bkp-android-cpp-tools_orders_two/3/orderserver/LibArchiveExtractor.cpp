...
ArchiveMetadata LibArchiveExtractor::findMetadataInArchive(const QString &sourceFilePath) {
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        // Normalize name: extract filename regardless of directory path
        ...
        if (normalizedName == "username.txt" && !foundUsername) {
            ...
            // MODIFIED: Read content and split by common line endings
            QString contentStr = QString::fromUtf8(content);
            // Split using QRegularExpression to handle \r\n, \r, or \n as delimiters, skipping empty parts
            metadata.usernames = contentStr.split(QRegularExpression("[\\r\\n]+"), Qt::SkipEmptyParts);
            qInfo() << "Metadata found: Usernames count=" << metadata.usernames.count();
            foundUsername = true;
        }
            ...
            // MODIFIED: Read content and split by common line endings
            QString contentStr = QString::fromUtf8(content);
            metadata.categories = contentStr.split(QRegularExpression("[\\r\\n]+"), Qt::SkipEmptyParts);
            qInfo() << "Metadata found: Categories count=" << metadata.categories.count();
            foundCategory = true;
        } else {
...
QJsonObject LibArchiveExtractor::extractAndProcessTarXzFile(const QString &sourceFilePath,
    ...
    // MODIFIED: Check the lists for emptiness
    if (metadata.usernames.isEmpty()) {
    ...
    // Determine primary user/category (first element) for the main 'data' object
    ...
    dataObject["username"] = primaryUsername; // MODIFIED: Use the first item
    dataObject["name"] = baseName;
    dataObject["category"] = primaryCategory; // MODIFIED: Use the first item
    ...
    struct archive_entry *entry;
    int mediaFileIndex = 0; // MODIFIED: Start index at 0
    ...
    while (r == ARCHIVE_OK) {
        ...
        // Determine the corresponding username and category for this index
        // Use modulo to cycle through the available list items if the media files exceed the list size
        const int userIndex = mediaFileIndex % usernamesList.count();
        const int catIndex = mediaFileIndex % categoriesList.count();
        ...
        QString entryName = baseName + QString::number(mediaFileIndex + 1); // Use +1 for 1-based naming
        nameArray.append(entryName);
        categoryArray.append(currentCategory); // MODIFIED: Use currentCategory
        usernameArray.append(currentUsername); // MODIFIED: Use currentUsername
        ...
        descriptionArray.append("File extracted: " + originalFileName + " by " + currentUsername); // MODIFIED: Use currentUsername
        createdAtArray.append(QDateTime::currentDateTimeUtc().toString(Qt::ISODate));

        if (mediaFileIndex == 0) { // Index is 0-based
        ...
        if (!addedCategories.contains(currentCategory)) { // MODIFIED: Use currentCategory
            QUuid categoryUuid = QUuid::createUuid();

            categoryIdArray.append(categoryUuid.toString(QUuid::WithoutBraces));
            categoryNameArray.append(currentCategory); // MODIFIED: Use currentCategory
            ...
            addedCategories.insert(currentCategory); // MODIFIED: Use currentCategory
        }
    ...
    queryInner["sql"] = "username = '" + primaryUsername + "'"; // MODIFIED: Use the single primary user

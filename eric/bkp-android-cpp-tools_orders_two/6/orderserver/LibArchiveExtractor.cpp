...
QJsonObject LibArchiveExtractor::extractAndProcessTarXzFile(const QString &sourceFilePath,
    ...
    // This assumes the first entry is the primary record for the submission
    ...
    dataObject["ids_id"] = primaryIdsId; // USED VALUE FROM FILE (FIRST ENTRY)
    ...
    while (r == ARCHIVE_OK) {
        ...
        // FIX: Extract IDS_ID using index, as requested
        ...
        idsIdArray.append(currentIdsId); // FIXED: Used indexed value from file
        ...
        if (!addedCategories.contains(currentCategory)) {
            ...
            categoryUsernameArray.append(primaryUsername);
            categoryCreatedAtArray.append(QDateTime::currentDateTimeUtc().toString(Qt::ISODate));
            categoryIdsIdArray.append(currentIdsId); // FIXED: Used indexed value from file
    ...
    // FIX: Only one request_id at the root level
    ...
    // FIX: Removed request_id from queryOuter to prevent duplicate key error
    queryOuter["action"] = "fetch_by_query";
    queryOuter["entity"] = "ids";
    queryOuter["criteria"] = queryInner; // Uses "criteria" to avoid clash with root "query"

...
ArchiveMetadata LibArchiveExtractor::findMetadataInArchive(const QString &sourceFilePath) {
    ...
    bool foundActionDue = false; // NEW FLAG
    ...
    while (archive_read_next_header(a, &entry) == ARCHIVE_OK) {
        ...
        isMetadata |= readAndSplit(metadata.actionDues, foundActionDue, "actiondue.txt"); // NEW
        ...
        if (foundUsername && foundCategory && foundItems && foundPrice && foundSizeOrQuantity && foundManufacturer && foundDescription && foundActionDue) {
            break;
        }
...
QJsonObject LibArchiveExtractor::extractAndProcessTarXzFile(const QString &sourceFilePath,
    ...
    const QStringList actionDuesList = metadata.actionDues; // NEW LIST
    ...
    const QString primaryActionDue = actionDuesList.first(); // NEW PRIMARY VALUE
    ...
    QJsonArray itemsIdArray, idsIdArray, uuidArray, mediaFileArray, nameArray, categoryArray,
        ...
        manufacturerArray, priceArray, descriptionArray, createdAtArray, actionDueArray; // NEW ARRAY
    ...
    dataObject["actiondue"] = primaryActionDue; // NEW KEY
    ...
    while (r == ARCHIVE_OK) {
        ...
        // --- NEW FILE INDEXING LOGIC (1-based index from file name) ---

        QFileInfo originalFileInfo(originalFileName);
        QString fileNameWithoutExtension = originalFileInfo.baseName();

        // Regex to find a leading number (e.g., '001' in '001.png' or '1' in '1_item.jpg')
        ...
        if (match.hasMatch()) {
            ...
            if (!ok || fileIndex < 1) {
                fileIndex = -1; // Invalid number or zero
            }
        }
        ...
        // Check if the calculated index is within bounds of the shortest/most critical metadata list
        ...
        // ... (archive_write_header and archive_write_data_block logic remains here)
        ...
        // Fetch current indexed values using listIndex. Modulo is technically redundant now but kept for safety.
        ...
        actionDueArray.append(currentActionDue); // NEW APPEND
    ...
    extractedItemsData["actiondue"] = actionDueArray; // NEW KEY

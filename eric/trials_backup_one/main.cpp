...
// At program startup, before Wt server / Wt::WApplication creations:
    ...
    QString wtTmpDir = QStandardPaths::writableLocation(QStandardPaths::AppDataLocation);
    if (wtTmpDir.isEmpty()) {
        wtTmpDir = QStandardPaths::writableLocation(QStandardPaths::CacheLocation);
    }
  ...
  //  QObject::connect(&app, &QCoreApplication::aboutToQuit, &dbManager, &DatabaseManager::closeDatabase);
    ...
    //                  mgr, &ServerManager::stopAllServers);

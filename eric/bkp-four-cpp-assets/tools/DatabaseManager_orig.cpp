...
bool DatabaseManager::saveDatabase()
{
    ...
    return QFile::copy(internalPath + "/data.db",
                       externalPath + "/digitalbloom_database_" + QDateTime::currentDateTime().toString("yyyy-MM-dd hh:mm:ss")+ ".db");
}
...
bool DatabaseManager::tableExists(const QString &tableName)
{
    ...
    else
    {
        ...
        {
            checkTable.exec("SELECT * FROM information_schema.tables WHERE table_schema = 'digitalbloom' AND table_name = '" + tableName + "' LIMIT 1;");

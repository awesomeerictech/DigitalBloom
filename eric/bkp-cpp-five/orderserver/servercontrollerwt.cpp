...
const std::string closeMenuText = "<i class='fa fa-bars' aria-hidden='true'></i> Close menu";
    ...
    : Wt::WApplication(myenv),openMenuButton_(nullptr),menuOpen_(false)
  {
    ...
    //  client_->setMaximumResponseSize(0);
      ...
      massuploader = std::make_shared<DataUploader>();
      extractionbridge = std::make_shared<LibArchiveQmlBridge>();
    //  networkutils->client_->bodyDataReceived().connect(networkutils.get(), &DataNet::mybodyDataReceived);
      ...
      tpl_ = new Wt::WTemplate();
      progress_ = new Wt::WText();
      log_ = new Wt::WContainerWidget();
      drop_ = new Wt::WFileDropWidget();
      //loadTimer_ = new Wt::WTimer();
      // container_ = new Wt::WContainerWidget();
      // div2_ = new Wt::WContainerWidget();
      // div3_ = new Wt::WContainerWidget(); // This holds the actual cards

      // container_ = new Wt::WContainerWidget();
      // container_->setStyleClass("album py-5 bg-light");

      // div2_ = new Wt::WContainerWidget();
      // div2_->setStyleClass("container");

      // div3_ = new Wt::WContainerWidget();
      ...
      Wt::WApplication *app = Wt::WApplication::instance();
       ...
       auto str1 = Wt::WString::tr("appview-view7-placehold");
      ...
      // qInfo() << "helppageanchor2_ clicked()" << Qt::endl;
      ...
      //   openMenuButton_->hide();
         ...
         app->messageResourceBundle().use(app->appRoot() + "filedrop");
       ...
       //  app->useStyleSheet("style/myfiledrop.css");
         app->useStyleSheet("style/customfiledrop.css");
         app->useStyleSheet("resources/Font-Awesome/css/fontawesome.min.css");
       //  app->useStyleSheet("style/mycustom.scss");
        ...
        arrowleftnewitemanchor_->clicked().connect([=] {
          ...
          //qInfo() << "New Item File Upload" << Qt::endl;
        ...
        arrowleftupdateitemanchor_->clicked().connect([=] {
         ...
         //qInfo() << "Update Item File Upload" << Qt::endl;
      ...
      //  std::unique_ptr<Wt::WAnchor> arrowlefexittupdateitemanchor(arrowleftexitupdateitemanchor_);
        ...
        // notifytimer3->start();
         ...
         //qInfo() << "signouticonanchor1_ clicked()" << Qt::endl;
       ...
       //  categoriesbtn_->setStyleClass("btn btn-secondary");
        ...
        /* std::unique_ptr<Wt::WPushButton> categoriesbtn(categoriesbtn_);
         ...
         searchitems_->textInput().connect([=] {
          ...
          qInfo() << "itemsmodel count "  << itemsstandardmodel->rowCount() << Qt::endl;
          qInfo() << "carouselmodel count "  << carouselstandardmodel->rowCount() << Qt::endl;
          if(itemsstandardmodel->rowCount()>0 && carouselstandardmodel->rowCount()>0) {
          if(itemsfilter->rowCount() > 0 && carouselfilter->rowCount() > 0) {


         // qInfo() << "Filters not empty! "  << Qt::endl;
        ...
        allitems_->bindWidget("myheader1", std::move(header1));
         ...
         auto viewdropme = Wt::WString::tr("productuploads");
         tpl_->setTemplateText(viewdropme);
         drop_ = tpl_->bindNew<Wt::WFileDropWidget>("dropwidget");
         drop_->setDropIndicationEnabled(true);
        // drop_->setGlobalDropEnabled(true);
         drop_->setAcceptDirectories(true);
         drop_->drop().connect(this, &MapperSoftware::handleDrop);
         drop_->newUpload().connect(this,&MapperSoftware::updateProgressListener);
         drop_->uploaded().connect(this, &MapperSoftware::saveFile);
         drop_->uploadFailed().connect(this, &MapperSoftware::failed);
         drop_->tooLarge().connect(this, &MapperSoftware::tooLarge);
         auto selectFileBtn = tpl_->bindNew<Wt::WPushButton>("select-file-btn", "Select File");
         selectFileBtn->setStyleClass("btn btn-primary btn-lg");
         selectFileBtn->clicked().connect(drop_, &Wt::WFileDropWidget::openFilePicker);
         auto selectDirBtn = tpl_->bindNew<Wt::WPushButton>("select-dir-btn", "Select Directory");
         selectDirBtn->setStyleClass("btn btn-primary btn-lg");
         selectDirBtn->hide();
         selectDirBtn->clicked().connect(drop_, &Wt::WFileDropWidget::openDirectoryPicker);

         progress_ = tpl_->bindNew<Wt::WText>("progress-msg");
         auto abort = tpl_->bindNew<Wt::WPushButton>("abort-btn", "abort");
         abort->setStyleClass("btn btn-primary btn-lg");
         abort->clicked().connect(this, &MapperSoftware::cancelUpload);
         tpl_->setCondition("if:progress", false);

         log_ = tpl_->bindNew<Wt::WContainerWidget>("log");
         ...
         std::unique_ptr<Wt::WTemplate> ourfiledrop(tpl_);
         tpl_->hide();
         ...
         allsettings_->bindWidget("ourfiledrop", std::move(ourfiledrop));
         ...
         execbtn->clicked().connect([=] {
          ...
          if(ordersmap.count()>0) {
           ...
           for (int i = 0; i < actualvalues.size(); ++i) {
              ...
              //  qInfo()  << "itemsid QList<QVariant> requestpg is: " << actualvalues.at(i).toString()  << Qt::endl;
              reqcounter++;
...
// https://0.0.0.0:8180/?_=/my-items/all-items&business=ericm
 // https://0.0.0.0:8180?business=ericm
       ...
       if(QString::fromStdString(parameter0).isEmpty() || QString::fromStdString(parameter0).isNull()) {

        str1 = Wt::WString::tr("appview-view7-placehold");
        allitems_->setTemplateText(str1);
       ...
       qInfo() <<"Parameter exists" << Qt::endl;
...
//qInfo() << "reqcounter final is: " << reqcounter << Qt::endl;
//qInfo() << "makereqcount final is: " << makereqcount << Qt::endl;
...
/*  QMultiMap<QString, QVariant>::iterator j = ordersmap.find("itemsid");
    ...
    qInfo() << "itemsid requestpg  is: " << j.value().toString() << Qt::endl;
    isnew = false;
     ++j;
...
// void MapperSoftware::clearCards()
// {
//     // Clear the container holding the cards before starting a new cycle


//     // if (div3_ && div2_ && container_) {
//     //     div3_->clear();
//     //     container_->clear();
//     //     div2_->clear();
//     //     // Since we are clearing, we also need to re-add the container to ensure it stays in the template
//     //     // This assumes 'container_' is the top-level template binding point
//     //     // std::unique_ptr<Wt::WContainerWidget> mycontainer(container_);
//     //     // allitems_->bindWidget("items", std::move(mycontainer));
//     // }

//     // Wt::WApplication::instance()->enableUpdates(true);
//     // Wt::WApplication::instance()->triggerUpdate();

//     // Defensive: ensure we have a parent div2_
//     if (div3_) {
//         qInfo() << "loadNextBatch: clearing old cards before rendering new batch";
//         div3_->clear();
//         container_->clear();
//         div2_->clear();
//         renderedUuids_.clear();
//         // if you replaced div3_ with a new container earlier, you can also do that here
//     }
...
// // Call this once (e.g. at startup)
// void MapperSoftware::ensureContainers()
// {
//     if (container_) {
//            // containers already created and bound
//            return;
//        }

//        // Create a new tree and bind it once
//        auto container = std::make_unique<Wt::WContainerWidget>();
//        container->setStyleClass("album py-5 bg-light");

//        auto div2 = std::make_unique<Wt::WContainerWidget>();
//        div2->setStyleClass("container");

//        auto div3 = std::make_unique<Wt::WContainerWidget>();
//        div3->setStyleClass("row row-cols-1 row-cols-sm-2 row-cols-md-3 g-3");

//        // set member non-owning handles BEFORE moving
//        container_ = container.get();
//        div2_ = div2.get();
//        div3_ = div3.get();

//        div2_->addWidget(std::move(div3));
//        container_->addWidget(std::move(div2));
...
// void MapperSoftware::rebuildContainers()
// {
//     // Create new nodes
//     auto container = std::make_unique<Wt::WContainerWidget>();
//     auto div2 = std::make_unique<Wt::WContainerWidget>();
...
//     // compose
//     div2->addWidget(std::move(div3));
//     container->addWidget(std::move(div2));

//     // bind into template (this takes ownership of the tree)
//     auto str1 = Wt::WString::tr("appview-view7");
//     allitems_->setTemplateText(str1);
//     allitems_->bindWidget("items", std::move(container));

//     // Now update member handles to point to the *new* live widgets.
//     // IMPORTANT: find the new nodes through the template or by saving them before move:
//     // If you need to keep handles, set them BEFORE std::move as in approach A.
// }


// void MapperSoftware::startLazyLoad()
// {
//     // --- Model Setup (From your original code) ---
//     // Model setup (do this every start in case model changed)
//       itemsfilter->setSourceModel(itemsstandardmodel);
//       itemsfilter->setDynamicSortFilter(true);
//       itemsfilter->setFilterKeyColumn(6);
//       itemsfilter->setFilterRole(Wt::ItemDataRole::Display);

//       totalRows_ = itemsfilter->rowCount();
//       qInfo() << "startLazyLoad: total rows:" << totalRows_ << Qt::endl;

//       // Ensure containers exist and are bound exactly once
//       if(!container_) {
//           ensureContainers();
//       }

//       // Reset indices and rendered set on fresh start
//       currentIndex_ = 0;
//       renderedUuids_.clear();

//       // Timer connect only once (prevent multiple connections)
//       if (!loadTimerConnected_) {
//           loadTimer_->timeout().connect(this, &MapperSoftware::loadNextBatch);
//           loadTimerConnected_ = true;
//       }

//       // Start timer if there are items
//       if (totalRows_ > 0) {
//           // restart timer interval (if changed)
//           loadTimer_->setInterval(std::chrono::milliseconds(timerInterval_));
//           // Load the first batch once (do not call it twice)
//           loadNextBatch();
//           if (!loadTimer_->isActive()) {
//               loadTimer_->start();
//           }
//       } else {
...
// void MapperSoftware::loadNextBatch()
// {
//     // prevent reentry if already running: stop timer while loading
//         if (loadTimer_->isActive()) {
//             loadTimer_->stop();
//         }

//         if (!div3_) {
//             qWarning() << "loadNextBatch: div3_ is null, aborting." << Qt::endl;
//             return;
//         }

//         // If we've exhausted rows
//         if (currentIndex_ >= totalRows_) {
//             // Decide whether to cycle or stop
//             if (totalRows_ > pageSize_) {
//                 // Start a fresh cycle
//                 currentIndex_ = 0;
//                 clearCards();    // clear the view and rendered set
//                 qInfo() << "All items displayed, starting new cycle." << Qt::endl;
//             } else {
//                 // total <= pageSize_ -> show once and stop
//                 qInfo() << "Total items <= pageSize, stopping." << Qt::endl;
//                 removetpl(allitems_, "myspinner1");
//                 return;
//             }
//         }

//         // Emit optional signal before loading
//         batchLoadTriggered_.emit(currentIndex_);

//         int startRow = currentIndex_;
//         int endRow = std::min(totalRows_, currentIndex_ + pageSize_);

//         qInfo() << "Loading batch rows:" << startRow << "to" << (endRow - 1) << Qt::endl;


//         for (int row = startRow; row < endRow; ++row) {
//               // Render card (this must not rebind or move the top-level containers)
//               renderCard(row);
//           }


//         currentIndex_ = endRow;
...
//         // hide spinner after first load
//         removetpl(allitems_, "myspinner1");
// }

// void MapperSoftware::renderCard(int row)
// {



//     if (!div3_) {
//             qWarning() << "renderCard: div3_ is null, cannot add card." << Qt::endl;
//             return;
//         }

//         // obtain uuid again to be sure (defensive)
//         Wt::cpp17::any uuidAny = itemsfilter->data(row, 12);
//         QString uuid = QString::fromStdString(Wt::cpp17::any_cast<Wt::WString>(uuidAny).toUTF8());

//         // double-check duplicate guard
//         if (renderedUuids_.contains(uuid)) {
//             qInfo() << "renderCard: uuid already rendered:" << uuid << Qt::endl;
//             return;
//         }

//           int cardIdNum = ++s_cardCounter;
//           auto cardId = Wt::WString::fromUTF8("card-%1").arg(cardIdNum);
//           auto appview8_ = std::make_unique<Wt::WTemplate>();
//           auto strview1 = Wt::WString::tr("appview-view8");
//           appview8_->setTemplateText(strview1);
//           appview8_->bindString("cardId", cardId, Wt::TextFormat::Plain);
//           auto paragraphtext_ = std::make_unique<Wt::WTemplate>();
//           auto str4 = Wt::WString::tr("appview-view4");
//           paragraphtext_->setTemplateText(str4);
//           Wt::cpp17::any mydatades = itemsfilter->data(row,2);
//           Wt::WString datades = Wt::cpp17::any_cast<Wt::WString>(mydatades);
//           Wt::cpp17::any mydataitemname = itemsfilter->data(row,6);
//           Wt::WString dataitemname = Wt::cpp17::any_cast<Wt::WString>(mydataitemname);
//           Wt::cpp17::any mydataprice = itemsfilter->data(row,7);
//           Wt::WString dataprice = Wt::cpp17::any_cast<Wt::WString>(mydataprice);
//           Wt::cpp17::any mydatasize = itemsfilter->data(row,8);
//           Wt::WString datasize = Wt::cpp17::any_cast<Wt::WString>(mydatasize);
//           Wt::cpp17::any mydatastandardized = itemsfilter->data(row,9);
//           Wt::WString datastandardized = Wt::cpp17::any_cast<Wt::WString>(mydatastandardized);
//           Wt::cpp17::any mydataquantity = itemsfilter->data(row,8);
//           Wt::WString dataquantity = Wt::cpp17::any_cast<Wt::WString>(mydataquantity);
//           Wt::cpp17::any mydatacategory = itemsfilter->data(row,0);
//           Wt::WString datacategory = Wt::cpp17::any_cast<Wt::WString>(mydatacategory);
//           Wt::cpp17::any mydatastandardizedmedia  = itemsfilter->data(row,10);
//           Wt::WString datastandardizedmedia = Wt::cpp17::any_cast<Wt::WString>(mydatastandardizedmedia);
//           Wt::cpp17::any mydatauuid  = itemsfilter->data(row,12);
//           Wt::WString datauuid = Wt::cpp17::any_cast<Wt::WString>(mydatauuid);
//           std::string paragraphtext = datades.toUTF8();
//           Wt::cpp17::any mydatausername = itemsfilter->data(row,11);
...
//           Wt::cpp17::any mydataitemuuid = itemsfilter->data(row,3);
//           Wt::WString dataitemuuid = Wt::cpp17::any_cast<Wt::WString>(mydataitemuuid);
//           Wt::cpp17::any mydatamanufacturer = itemsfilter->data(row,4);
//           Wt::WString datamanufacturer = Wt::cpp17::any_cast<Wt::WString>(mydatamanufacturer);
//           Wt::cpp17::any mydatamedia = itemsfilter->data(row,5);
...
//           qInfo() << "media src from item filter: " << QString::fromStdString(mysrc) << Qt::endl;
...
//               //qInfo() << "mysrc is: " << QString::fromStdString(mysrc) << Qt::endl;
//               //qInfo() << "card is: " << card << Qt::endl;
...
//                  //  qInfo() << "Proceed to Update an item... "  << Qt::endl;
...
//                    scope3a_->show();
//                    scope3_->hide();
//                  //  qInfo() << "btnscope3_ clicked()!" << Qt::endl;
//                    if(timercarousel->isActive()) {
...
//       //  qInfo()  << "Adding itemuuid in DS!" << Qt::endl;
//         ordersmap.insert("itemsid",QString::fromStdString(dataitemuuid.toUTF8()));
...
//     //   qInfo()  << "itemuuid already in DS!" << Qt::endl;
...
//       //  qInfo()  << "ids_id QList<QVariant> is: " << ids_idvalues.at(i).toString()  << Qt::endl;
...
//        // qInfo()  << "recipient QList<QVariant> is: " << recipientvalues.at(i).toString()  << Qt::endl;
...
//        // qInfo()  << "deliverer QList<QVariant> is: " << deliverervalues.at(i).toString()  << Qt::endl;
...
//       //  qInfo()  << "locationto QList<QVariant> is: " << locationtovalues.at(i).toString()  << Qt::endl;
...
//      //   qInfo()  << "locationfrom QList<QVariant> is: " << locationfromvalues.at(i).toString()  << Qt::endl;
...
//       //  qInfo()  << "itemsid QList<QVariant> is: " << actualvalues.at(i).toString()  << Qt::endl;
...
//        QMultiMap<QString, QVariant>::iterator j = ordersmap.find("itemsid");
//        while (j != ordersmap.end() && j.key() == "itemsid") {
//       // qInfo() << "itemsid   is: " << j.value().toString() << Qt::endl;
//        isnew = false;
//         ++j;
...
//                  //  qInfo() << "btnscope3a_ clicked()!" << Qt::endl;
//                    scope3a_->hide();
//                    scope3_->show();
...
//                  /*  QMultiMap<QString, QVariant>::iterator j = ordersmap.find("itemsid");
//                    while (j != ordersmap.end() && j.key() == "itemsid") {
//                    qInfo() << "itemsid after remove is: " << j.value().toString() << Qt::endl;
//                     ++j;
...
//           else if(!(std::regex_match(myinputtxt1, std::regex(spattern1))) && !(datastandardizedmedia.toUTF8()=="") ) {
...
//           auto btnId = Wt::WString::fromUTF8("btn-update-%1").arg(cardIdNum);
//           appview8_->bindString("btnUpdateId", btnId, Wt::TextFormat::Plain);
...
//           // // at top of renderCard(), after uuid fetch/duplicate guard
//           // qInfo() << "renderCard ENTER: div3_=" << (quintptr)div3_
//           //         << " div3 count BEFORE add:" << (div3_ ? div3_->count() : -1);

//           // // right after you create the template but BEFORE move:
//           // qInfo() << "renderCard: created appview8 ptr=" << (quintptr)appview8_.get() << " row=" << row;

//           // after adding to div3_
...
//                  //qInfo() << "helppageanchor1_ clicked()" << Qt::endl;
...
//                    QString theuuid = QString::fromStdString(Wt::cpp17::any_cast<Wt::WString>(mydatauuid).toUTF8());


//                    if (renderedUuids_.contains(theuuid)) {
//                        qInfo() << "Skipping duplicated uuid:" << uuid << Qt::endl;
//                    }

//                    else {

//                        renderedUuids_.insert(theuuid);
//                    }


//                    // mark as rendered before rendering to avoid race re-adds
...
// Start File Drop functions


void MapperSoftware::handleDrop(std::vector<Wt::WFileDropWidget::File *> files)
{
  for (unsigned i=0; i < files.size(); i++) {
    ...
    if (file->directory()) {
      ...
      auto dir = dynamic_cast<Wt::WFileDropWidget::Directory*>(file);
    ...
    addLogLine("New item uploaded: " + file->path());

   // qInfo()<< "Uploaded dropped file is: " << QString::fromUtf8()

  }

  if (icons_.size() >= MAX_DROPS) {
    drop_->setAcceptDrops(false);
    addLogLine("Maximum number of item upload reached, disabling uploads.");
  }
}

void MapperSoftware::cancelUpload()
{
...
void MapperSoftware::tooLarge(Wt::WFileDropWidget::File *file, ::uint64_t)
{
...
void MapperSoftware::failed(Wt::WFileDropWidget::File *file)
{
  getIcon(file)->addStyleClass("invalid");
...
void MapperSoftware::saveFile(Wt::WFileDropWidget::File *file)
{
  ...
  if (!QDir(wtTmpDir).exists()) {
      if (!QDir().mkpath(wtTmpDir)) {
          qWarning() << "Failed to create WT For Uploads tmp dir:" << wtTmpDir;
      }
  }

  auto saveName = Wt::cpp17::filesystem::path(wtTmpDir.toStdString()) / file->path();
  ...
  if (dirFiles_.find(file) != dirFiles_.end()) {
    Wt::WFileDropWidget::Directory *dir = dirFiles_[file];
    const std::vector<Wt::WFileDropWidget::File*>& contents = dir->contents();
...
void MapperSoftware::updateProgressListener()
{
  ...
  if (static_cast<size_t>(drop_->currentIndex()) < drop_->uploads().size()) {
    ...
    file->dataReceived().connect(this, &MapperSoftware::showProgress);
...
void MapperSoftware::showProgress(::uint64_t current, ::uint64_t total)
{
  Wt::WFileDropWidget::File *file = drop_->uploads()[drop_->currentIndex()];
...
Wt::WContainerWidget* MapperSoftware::getIcon(Wt::WFileDropWidget::File *file)
{
  if (dirFiles_.find(file) != dirFiles_.end()) {
...
void MapperSoftware::addLogLine(const std::string& msg)
{
  log_->addNew<Wt::WText>("<div>" + msg + "</div>");
}
...
void MapperSoftware::mytimerendpointuser() {
    ...
    //qInfo() << "timerendpointuser timer timeout" << Qt::endl;
    //allitems_->bindEmpty("items");
...
//qInfo() << "Data mycarousel is: " << data << Qt::endl;
...
//   //qInfo() << "mynewjson3 is: " << qPrintable(mynewjson3) << Qt::endl;
...
//qInfo() << "rem is: " << rem << Qt::endl;
//   //qInfo() << "car3 is: " << car3 << Qt::endl;
...
qInfo() << "Attempting to log the media: " << QString::fromStdString(mediasrcaaa) << Qt::endl;
...
//qInfo() << "appview13aaa_ the count is: " << thecount << Qt::endl;
...
//qInfo() << "appview13aab_ the count is: " << thecount << Qt::endl;
...
//qInfo() << "appview13aac_  the count is: " << thecount << Qt::endl;
...
/*
 ...
 QString data = *str;
 //qInfo() << " myerrormessage is: " << data << Qt::endl;
...
//qInfo() << "mysuccessmessage is: " << data << Qt::endl;
...
if(!fs.is_open())
  {
   ...
   //qInfo() << "creating tokenfile" << Qt::endl;
     fs.close();
     ...
     if(fidsout) {
       ...
       //qInfo() << "idids is: " << myidsid << Qt::endl;
 ...
 //qInfo() << "tokenfile already exist now opening it for writing" << Qt::endl;
...
//qInfo() << "directory already exists! " << Qt::endl;
 std::fstream fs(tokenfile, std::ios::in);
...
void MapperSoftware::thedatauser() { // start thedatauser()
   ...
   // qInfo() << "The data user is: " << data << Qt::endl;
   ...
   /* if(!(timercarousel->isActive())) {
    ...
    //   //qInfo() << "mynewjson1 is: " << qPrintable(mynewjson1) << Qt::endl;
  ...
  //  std::cout << "ids csv output is: " << output1 << "\n\n";
    ...
    QString mynewjson2 = newdoc2.toJson(QJsonDocument::JsonFormat::Indented);
       //qInfo() << "mynewjson2 is: " << qPrintable(mynewjson2) << Qt::endl;
    ...
    ioptions3.quote_style(jsoncons::csv::quote_style_kind::nonnumeric);
    jsoncons::csv::encode_csv(j3, output3, ioptions3);
    qInfo() << "csv output3 is: " << QString::fromStdString(output3) << "\n\n";
    ...
    //qInfo() << "mynewjson4 is: " << qPrintable(mynewjson4) << Qt::endl;
    ...
    //qInfo() << "mynewjsonlogi is: " << qPrintable(mynewjsonlogi) << Qt::endl;
    ...
    QString mynewjson5 = newdoc5.toJson(QJsonDocument::JsonFormat::Indented);
     //qInfo() << "mynewjson5 is: " << qPrintable(mynewjson5) << Qt::endl;
    ...
    //qInfo() << "mynewjson6 is: " << qPrintable(mynewjson6) << Qt::endl;
    ...
    //qInfo() << "mynewjson8 is: " << qPrintable(mynewjson8) << Qt::endl;
   ...
   // ordersfilter->setFilterRegExp(std::make_unique<std::regex>(spattern));
    ...
    int ordersrow = ordersfilter->rowCount();
     ...
     if((idsstandardmodel->columnCount() && idsstandardmodel->rowCount())==0) {
      ...
      allitems_->bindEmpty("bizphone");
        ...
        /*  qInfo() << "dataidids is: " << QString::fromStdString(dataidids.toUTF8()) << Qt::endl;
          qInfo() << "dataidphone is: " << QString::fromStdString(dataidphone.toUTF8()) << Qt::endl;
          qInfo() << "dataiddate is: " << QString::fromStdString(dataiddate.toUTF8()) << Qt::endl;
          qInfo() << "dataidsub is: " << QString::fromStdString(dataidsub.toUTF8()) << Qt::endl;
          qInfo() << "dataidblacklist is: " << QString::fromStdString(dataidblacklist.toUTF8()) << Qt::endl;
          qInfo() << "dataidbusiness is: " << QString::fromStdString(dataidbusiness.toUTF8()) << Qt::endl;
          qInfo() << "dataidresponsibiities is: " << QString::fromStdString(dataidresponsibiities.toUTF8()) << Qt::endl;
          qInfo() << "dataidauthority is: " << QString::fromStdString(dataidauthority.toUTF8()) << Qt::endl;
          qInfo() << "dataidverification is: " << QString::fromStdString(dataidverification.toUTF8()) << Qt::endl;
          qInfo() << "dataidpolicy  is: " << QString::fromStdString(dataidpolicy.toUTF8()) << Qt::endl;
          qInfo() << "dataidadname  is: " << QString::fromStdString(dataidadname.toUTF8()) << Qt::endl;
          qInfo() << "dataidavatarname  is: " << QString::fromStdString(dataidavatarname.toUTF8()) << Qt::endl;
          qInfo() << "dataididcardfront  is: " << QString::fromStdString(dataididcardfront.toUTF8()) << Qt::endl;
          qInfo() << "dataididcardback  is: " << QString::fromStdString(dataididcardback.toUTF8()) << Qt::endl;
          qInfo() << "dataidemail  is: " << QString::fromStdString(dataidemail.toUTF8()) << Qt::endl;
          qInfo() << "dataidusername  is: " << QString::fromStdString(dataidusername.toUTF8()) << Qt::endl;
          */
        ...
        /*  qInfo() << "businesssubscription is: " << businesssubscription << Qt::endl;
          qInfo() << "businessblacklist is: " << businessblacklist << Qt::endl;
          qInfo() << "businessresponsibilities is: " << businessresponsibilities << Qt::endl;
          qInfo() << "businesspolicy is: " << businesspolicy << Qt::endl;
          qInfo() << "businessverification is: " << businessverification << Qt::endl; */
    ...
    if(catrows>0 && !(businesssubscription > 1 || businessblacklist > 0 || businessverification > 3) && (userdue==loggeduserdue)) {
       ...
       //qInfo() << "catname is: " << QString::fromStdString(catname.toUTF8()) << Qt::endl;
     ...
     //    mycategories[catname.toUTF8()] = catname.toUTF8();
       ...
       //  qInfo() << "deleteiconme clicked()" << Qt::endl;
     //   qInfo() << "Category is: " << QString::fromStdString(catname.toUTF8()) << Qt::endl;
     //   qInfo() << "Category id is: " << QString::fromStdString(catid.toUTF8()) << Qt::endl;
...
//qInfo() << "datadate orders is: " << QString::fromStdString(datadate.toUTF8()) << Qt::endl;
...
allrequests_->bindEmpty("myrequestsaction");
   ...
   // qInfo() << "itemsstandardmodel is empty! " << Qt::endl;
    auto container = std::make_unique<Wt::WContainerWidget>();
    container->setStyleClass("album py-5 bg-light");
...
// }
       ...
       //  std::string myinputtxt = "ericm";
        ...
        qInfo() << "column count is: " << itemsfilter->columnCount() << Qt::endl;
        qInfo() << "row count is: " << itemsfilter->rowCount() << Qt::endl;
   ...
   //     qInfo() << "column count is: " << itemsfilter->columnCount() << Qt::endl;
   //     qInfo() << "initial row count is: " << itemsfilter->rowCount() << Qt::endl;

   //     // Build outer containers like you had
   //     auto container = std::make_unique<Wt::WContainerWidget>();
   //     container->setStyleClass("album py-5 bg-light");
   ...
   //     // raw pointer to inner container (we will add cards into this)
   //     Wt::WContainerWidget *div3_ptr = div3_.get();

   //     div2_->addWidget(std::move(div3_));
   //     container->addWidget(std::move(div2_));
   //     // bind immediately so template shows spinner etc.
   //     allitems_->bindWidget("items", std::move(container));
   ...
   //     // single-shot timer to avoid overlapping chunks
   //     auto pageTimer = std::make_shared<Wt::WTimer>();
   //     pageTimer->setInterval(std::chrono::milliseconds{50}); // ms between chunks (tweakable)
   //     pageTimer->setSingleShot(true);

   //     // raw pointer to model for safe capture
   //     auto *filter = itemsfilter.get();

   //     // Debug: total number of rows we'll try to load (but we re-check each tick)
   //     qInfo() << "Starting paged load, pageSize =" << pageSize << Qt::endl;

   //     pageTimer->timeout().connect([this, pageTimer, currentIndex, div3_ptr, filter]() mutable {
   //         // Fetch the latest total rows each tick (model may change)
   //         int totalRows = filter->rowCount();
   //         int start = *currentIndex;
   //         if (start >= totalRows) {
   //             qInfo() << "No more rows to load (start >= totalRows):" << start << totalRows << Qt::endl;
   //             // done
   //             this->removetpl(allitems_, "myspinner1");
   //             return;
   //         }

   //         int end = std::min(start + pageSize, totalRows);
   //         qInfo() << "Loading chunk: start =" << start << " end =" << end << " total =" << totalRows << Qt::endl;

   //         // Process rows [start, end)
   //         for (int card = start; card < end; ++card) {

   //             // -----------------------------
   //             // BEGIN: original card-building code, using 'filter' and 'div3_ptr'
   //             // (I kept this intentionally identical to your original; only minor safe copies)
   //             // -----------------------------
   ...
   //             // IMPORTANT: use filter->data(), not itemsfilter->data(), inside lambda
   //             Wt::cpp17::any mydatades = filter->data(card,2);
   //             Wt::WString datades = Wt::cpp17::any_cast<Wt::WString>(mydatades);

   //             Wt::cpp17::any mydataitemname = filter->data(card,6);
   //             Wt::WString dataitemname = Wt::cpp17::any_cast<Wt::WString>(mydataitemname);

   //             Wt::cpp17::any mydataprice = filter->data(card,7);
   //             Wt::WString dataprice = Wt::cpp17::any_cast<Wt::WString>(mydataprice);

   //             Wt::cpp17::any mydatasize = filter->data(card,8);
   //             Wt::WString datasize = Wt::cpp17::any_cast<Wt::WString>(mydatasize);

   //             Wt::cpp17::any mydatastandardized = filter->data(card,9);
   //             Wt::WString datastandardized = Wt::cpp17::any_cast<Wt::WString>(mydatastandardized);

   //             Wt::cpp17::any mydataquantity = filter->data(card,8);
   //             Wt::WString dataquantity = Wt::cpp17::any_cast<Wt::WString>(mydataquantity);

   //             Wt::cpp17::any mydatacategory = filter->data(card,0);
   //             Wt::WString datacategory = Wt::cpp17::any_cast<Wt::WString>(mydatacategory);

   //             Wt::cpp17::any mydatastandardizedmedia  = filter->data(card,10);
   //             Wt::WString datastandardizedmedia = Wt::cpp17::any_cast<Wt::WString>(mydatastandardizedmedia);

   //             Wt::cpp17::any mydatauuid  = filter->data(card,12);
   //             Wt::WString datauuid = Wt::cpp17::any_cast<Wt::WString>(mydatauuid);

   //             std::string paragraphtext = datades.toUTF8();

   //             Wt::cpp17::any mydatausername = filter->data(card,11);
   //             Wt::WString datausername = Wt::cpp17::any_cast<Wt::WString>(mydatausername);
   ...
   //             Wt::cpp17::any mydataitemuuid = filter->data(card,3);
   //             Wt::WString dataitemuuid = Wt::cpp17::any_cast<Wt::WString>(mydataitemuuid);

   //             Wt::cpp17::any mydatamanufacturer = filter->data(card,4);
   //             Wt::WString datamanufacturer = Wt::cpp17::any_cast<Wt::WString>(mydatamanufacturer);

   //             Wt::cpp17::any mydatamedia = filter->data(card,5);
   //             Wt::WString datamedia = Wt::cpp17::any_cast<Wt::WString>(mydatamedia);

   //             std::string mysrc = QString::fromStdWString(networkutils->myurldue(MEDIAURL)).toStdString() + datamedia.toUTF8();
   //             qInfo() << "media src from item filter: " << QString::fromStdString(mysrc) << Qt::endl;

   //             // make safe copies for lambdas
   //             Wt::WString datades_copy = datades;
   //             Wt::WString dataitemname_copy = dataitemname;
   //             Wt::WString dataprice_copy = dataprice;
   //             Wt::WString datasize_copy = datasize;
   //             Wt::WString datastandardized_copy = datastandardized;
   //             Wt::WString dataquantity_copy = dataquantity;
   //             Wt::WString datacategory_copy = datacategory;
   //             Wt::WString datastandardizedmedia_copy = datastandardizedmedia;
   //             Wt::WString datauuid_copy = datauuid;
   //             Wt::WString datausername_copy = datausername;
   //             Wt::WString dataitemuuid_copy = dataitemuuid;
   //             Wt::WString datamanufacturer_copy = datamanufacturer;
   //             Wt::WString datamedia_copy = datamedia;
   //             std::string mysrc_copy = mysrc;
   //             std::string paragraphtext_std = paragraphtext;
   ...
   //                 if (loggeduserdue.isEmpty()) {
   //                     if (appvieweric_->isVisible()) {
   //                         if (accregister_) accregister_->hide();
   //                         if (accretrieve_) accretrieve_->hide();
   //                         if (accsetup_) accsetup_->show();
   //                         appvieweric_->hide();
   //                     }
   //                 } else if (!loggeduserdue.isEmpty() && loggeduserdue.toStdString() == datausername_copy.toUTF8()) {
   //                     doupdateitem = true;
   //                     retainmedianame = datamedia_copy;
   //                     retainuniqueid = dataitemuuid_copy;
   //                     int index = combocategoryupdate->findText(datacategory_copy);
   //                     combocategoryupdate->setCurrentIndex(index);
   //                     updateitemname->setText(dataitemname_copy);
   //                     updatequantity->setText(dataquantity_copy);
   //                     updateprice->setText(dataprice_copy);
   //                     updateitemdescription->setText(datades_copy);
   //                     updatestandardized->setText(datastandardized_copy);
   //                     updatestandardizedmedia->setText(datastandardizedmedia_copy);
   //                     updateitemuuid->setText(datauuid_copy);
   //                     updatemanufacturer->setText(datamanufacturer_copy);
   //                     updatetemplateformview_->updateModel(myformmodelupdate_.get());
   //                     updatetemplateformview_->updateView(myformmodelupdate_.get());
   ...
   //                     QVariantMap info;
   //                     info.insert("user", loggeduserdue);
   //                     info.insert("itemsid", QString::fromStdString(dataitemuuid_copy.toUTF8()));
   //                     info.insert("name", QString::fromStdString(dataitemname_copy.toUTF8()));
   //                     info.insert("category", QString::fromStdString(datacategory_copy.toUTF8()));
   ...
   //                         ordersmap.insert("locationto", reqlocationto);
   //                         ordersmap.insert("locationfrom", reqlocationfrom);
   //                         ordersmap.insert("itemsid", QString::fromStdString(dataitemuuid_copy.toUTF8()));
   //                     }

   //                     bool inmap = false;
   //                     QList<QVariant> values = ordersmap.values("itemsid");
   //                     for (int i = 0; i < values.size(); ++i) {
   //                         if (values.at(i).toString() == QString::fromStdString(dataitemuuid_copy.toUTF8())) {
   //                             inmap = true;
   //                         }
   //                     }
   ...
   //                         ordersmap.remove("locationto", reqlocationto);
   //                         ordersmap.remove("locationfrom", reqlocationfrom);
   //                         ordersmap.remove("itemsid", QString::fromStdString(dataitemuuid_copy.toUTF8()));
   //                     } else {
   //                         ordersmap.remove("itemsid", QString::fromStdString(dataitemuuid_copy.toUTF8()));
   //                     }
   //                 } else {
   ...
   //             std::string match1 = "https://www.youtube.com/embed";
   //             std::string match2 = "https://www.youtube.com/v";
   //             std::string myinputtxt1 = datastandardizedmedia_copy.toUTF8();
   //             std::string spattern1 = "(" + match1 + ")(.*)";
   //             std::string spattern2 = "(" + match2 + ")(.*)";
   ...
   //             auto strviewordinaryimage1 = Wt::WString::tr("ordinaryimages");
   //             appviewordinaryimage1_->setTemplateText(strviewordinaryimage1);
   //             appviewordinaryimage1_->bindString("imgsrc", mysrc_copy, Wt::TextFormat::Plain);

   //             auto appviewordinaryimage2_ = std::make_unique<Wt::WTemplate>();
   //             auto strviewordinaryimage2 = Wt::WString::tr("ordinaryimages");
   //             appviewordinaryimage2_->setTemplateText(strviewordinaryimage2);
   //             appviewordinaryimage2_->bindString("imgsrc", mysrc_copy, Wt::TextFormat::Plain);
   //             auto appviewordinaryimage2 = appviewordinaryimage2_.get();

   //             auto appviewembedyoutube1_ = std::make_unique<Wt::WTemplate>();
   //             auto embedyoutube1 = Wt::WString::tr("youtubeembed1");
   //             appviewembedyoutube1_->setTemplateText(embedyoutube1);
   //             appviewembedyoutube1_->bindString("standardizedmedia", datastandardizedmedia_copy.toUTF8(), Wt::TextFormat::Plain);
   //             auto appviewembedyoutube1 = appviewembedyoutube1_.get();
   ...
   //             auto flashcontainer = std::make_unique<Wt::WContainerWidget>();
   //             auto flash = flashcontainer->addNew<Wt::WFlashObject>(datastandardizedmedia_copy.toUTF8());
   //             flash->setFlashParameter("allowFullScreen", "true");
   //             auto myposter1 = std::make_unique<Wt::WImage>(Wt::WLink(mysrc_copy));
   //             myposter1->setStyleClass("img-fluid card-img-top");
   //             myposter1->setAttributeValue("width", "100%");
   //             myposter1->setAttributeValue("height", "100%");
   //             flash->setAlternativeContent(std::move(myposter1));
   ...
   //             containervideo->setStyleClass("card");
   //             auto videoply = containervideo->addNew<Wt::WVideo>();
   //             videoply->addSource(Wt::WLink(mysrc_copy));
   //             auto myposter2 = std::make_unique<Wt::WImage>(Wt::WLink(datastandardizedmedia_copy.toUTF8()));
   //             myposter2->setStyleClass("img-fluid card-img-top");
   //             myposter2->setAttributeValue("width", "100%");
   //             myposter2->setAttributeValue("height", "100%");
   //             videoply->setAlternativeContent(std::move(myposter2));

   //             if (datastandardizedmedia_copy.toUTF8() == "later" || datastandardizedmedia_copy.toUTF8() == "null" || datastandardizedmedia_copy.toUTF8() == "nill" || datastandardizedmedia_copy.toUTF8() == "empty" || datastandardizedmedia_copy.toUTF8() == "N/A") {
   //                 appview8_->bindWidget("mediasrc", std::move(appviewordinaryimage1_));
   //             } else if (!(std::regex_match(myinputtxt1, std::regex(spattern1))) && !(datastandardizedmedia_copy.toUTF8() == "")) {
   //                 appview8_->bindWidget("mediasrc", std::move(appviewordinaryimage1_));
   //             } else if ((std::regex_match(myinputtxt1, std::regex(spattern1)))) {
   //                 appviewordinaryimage2->hide();
   //                 appview8_->bindWidget("mediasrc", std::move(appviewembedyoutube2_));
   //             } else if ((std::regex_match(myinputtxt1, std::regex(spattern2)))) {
   //                 appview8_->bindWidget("mediasrc", std::move(flashcontainer));
   //             } else if (datastandardizedmedia_copy.toUTF8() == "localvideo") {
   //                 appview8_->bindWidget("mediasrc", std::move(containervideo));
   //             } else {
   //                 appview8_->bindWidget("mediasrc", std::move(containervideo));
   //             }
   ...
   //             appview8_->bindString("smalltxt", smalltext, Wt::TextFormat::Plain);
   //             appview8_->bindString("para", paragraphtext_std, Wt::TextFormat::Plain);
   //             appview8_->bindString("standardized", datastandardized_copy.toUTF8(), Wt::TextFormat::Plain);
   //             appview8_->bindString("quantity", dataquantity_copy.toUTF8(), Wt::TextFormat::Plain);
   //             appview8_->bindString("category", datacategory_copy.toUTF8(), Wt::TextFormat::Plain);
   //             appview8_->bindString("itemname", dataitemname_copy.toUTF8(), Wt::TextFormat::Plain);
   //             appview8_->bindString("username", datausername_copy.toUTF8(), Wt::TextFormat::Plain);
   //             appview8_->bindString("manufacturer", datamanufacturer_copy.toUTF8(), Wt::TextFormat::Plain);
   //             appview8_->bindWidget("myparagraph", std::move(paragraphtext_));
   //             appview8_->bindWidget("mysmalltext", std::move(smalltext_));
   //             appview8_->bindWidget("myimagetemplate", std::move(imagetemplate_));
   //             appview8_->bindWidget("btn1", std::move(btnscope1_));
   ...
   //             div3_ptr->addWidget(std::move(appview8_));

   //             // -----------------------------
   //             // END: original card-building code
   //             // -----------------------------
   //         } // end for chunk

   //         // update current index and either restart timer or finish
   //         *currentIndex = end;
   //         qInfo() << "Chunk finished. new currentIndex =" << *currentIndex << " total =" << totalRows << Qt::endl;

   //         if (*currentIndex < totalRows) {
   //             // schedule next chunk
   //             pageTimer->start(); // single-shot -> restart explicitly
   //         } else {
   //             // done, cleanup & final UI binding
   //             pageTimer->stop();
   //             this->removetpl(allitems_, "myspinner1");
...
//QString * str = myerrormessage.get();
//QString data = *str;
//qInfo() << "error message is: " << qPrintable(data) << Qt::endl;
...
//qInfo() << "items_id is: " << results << Qt::endl;
//qInfo() << "name is: " << name << Qt::endl;
//qInfo() << "data theitemnew is: " << data << Qt::endl;
...
QString results =  mydearobj.value("items_id").toString();
//qInfo() << "Item id is: " << results << Qt::endl;
//qInfo() << "data is: " << data << Qt::endl;
...
Q_UNUSED(results)
//qInfo() << "data updatepassword is: " << data << Qt::endl;
//qInfo() << "idsid 1 updatepassword is: " << results << Qt::endl;
networkutils->getuseritems(userdue);
timergetdatauser->start();
...
QString results =  mydearobj.value("ids_id").toString();
Q_UNUSED(results)
//qInfo() << "idsid 2 updatephonenumber is: " << results << Qt::endl;
networkutils->getuseritems(userdue);
timergetdatauser->start();
...
//Q_UNUSED(results)
//qInfo() << "categories id is: " << results << Qt::endl;
if(!(results=="")) {
if(timercarousel->isActive()){
...
Q_UNUSED(results)
qInfo() << "order id is: " << results << Qt::endl;
if(!results.isEmpty()) {
...
Q_UNUSED(results)
//qInfo() << "deleted is: " << results << Qt::endl;
//qInfo() << "data is: " << data << Qt::endl;
...
Q_UNUSED(results)
//qInfo() << "data is: " << data << Qt::endl;
//qInfo() << "Phone number is: " << results << Qt::endl;
QVariantMap info;
...
Q_UNUSED(results)
//qInfo() << "Price is: " << results << Qt::endl;
//qInfo() << "data is: " << data << Qt::endl;
QVariantMap info;
...
QVariantMap MapperSoftware::myuserDatanew(QString mediafile) {
 ...
 if (myformmodelnew_->validate()) {
   ...
   //qInfo() << "Data to be uploaded item new is: " << QString::fromStdString(userDatanew().toUTF8()) << Qt::endl;
...
mytemplateformview_->updateView(myformmodelnew_.get());
//qInfo() << "myformmodelnew_ model validated!" << Qt::endl;
...
mytemplateformview_->updateView(myformmodelnew_.get());
//qInfo() << "myformmodelnew_ model was not validated!" << Qt::endl;
...
updatetemplateformview_->updateView(myformmodelupdate_.get());
//qInfo() << "myformmodelupdate_ model validated!" << Qt::endl;
...
fu->uploaded().connect([&] {
   ...
   for (const Wt::Http::UploadedFile & n : fu->uploadedFiles()) {
          ...
          //qInfo() << "content type is: " << QString::fromStdString(n.contentType()) << Qt::endl;
          //qInfo() << "spooled file name is: " << QString::fromStdString(n.spoolFileName()) << Qt::endl;
          //qInfo() << "uuid payload is: " << mypayload->newuuid() << Qt::endl;
           //qInfo() << "long date payload is: " << mypayload->longdatetime() << Qt::endl;
          //qInfo() << "newfile is: " << mypayload->newfilename() << Qt::endl;
           //qInfo() << "mime type is: " << mypayload->fileSuffixFromPath(QString::fromStdString(n.clientFileName())) << Qt::endl;
           fu->stealSpooledFile();
    ...
    //qInfo() << "doupload is: " << doupload << Qt::endl;
    ...
    //qInfo() << "content description is: " << QString::fromStdString(fu->contentDescription().toUTF8()) << Qt::endl;
...
uploadtemplate_->bindEmpty("uploadfile");
 //qInfo() << "timeouttimer3() running" << Qt::endl;
...
//qInfo() << "Menu text is: " << QString::fromStdString(myitem->text().toUTF8()) << Qt::endl;
...
//qInfo() << "in wt slot final json is: " << qPrintable(finaljson) << Qt::endl;
mypayload->processRequest(QByteArray::fromStdString(rawData));
QJsonObject mydearpayload = mypayload->m_requestJson.toObject();
//qInfo() << "in wt slot restserverErrordata  m_requestJson  is: " <<  mypayload->m_requestJson  << Qt::endl;
...
void MapperSoftware::handleSpecificPathChange(const std::string&)
{
  ...
  //qInfo() << "/account-set-up/register called! " << Qt::endl;
   ...
   //qInfo() << "/account-set-up/retrieve-pin called! " << Qt::endl;
...
//qInfo() << "/account-set-up/ called! " << Qt::endl;
...
/* if(myaccounts_)
    ...
    if(accsetup_)
     ...
     //qInfo() << "Top path is:  " << QString::fromStdString(app->internalPath()) << Qt::endl;

    }

    else if (app->internalPath() == "/my-items") {

      //qInfo() << "in /my-items my-items called! " << Qt::endl;
   ...
   //   qInfo() << "/my-items/all-items path called! " << Qt::endl;
      ...
      //qInfo() << "/login path called! " << Qt::endl;
   ...
   /*   if(appvieweric_->isVisible()) {
         ...
         //qInfo() << "/my-items/new-item called! " << Qt::endl;
      ...
      /*   if((idsstandardmodel->columnCount()>0 && idsstandardmodel->rowCount())>0) {
       ...
       //qInfo() << "/my-items/update-item called! " << Qt::endl;
        ...
        //qInfo() << "itemname is: " << QString::fromStdString(itemname) << Qt::endl;
     ...
     // qInfo() << "/ called! " << Qt::endl;
       ...
       // qInfo() << "other path " << QString::fromStdString(app->internalPath()) << " called!" << Qt::endl;
...
void MapperSoftware::thetimer1(bool timeout) {
 ...
 //qInfo() << "timer1 timeout is true" << Qt::endl;
  //   networkutils->getuseritems("ericm");
 }

else {

 //qInfo() << "timer1 timeout is false" << Qt::endl;
...
void MapperSoftware::thetimer2(bool timeout) {
    ...
    if(timeout) {
      ...
      //qInfo() << "timer2 timeout is true" << Qt::endl;
    }

   else {

      //qInfo() << "timer2 timeout is false" << Qt::endl;
...
void MapperSoftware::thetimer3(bool timeout) {
    ...
    if(timeout) {
      ...
      //qInfo() << "timer3 timeout is true" << Qt::endl;
    }

   else {

     //qInfo() << "timer3 timeout is false" << Qt::endl;
 ...
 void UserNameResource::handlenameresource(std::string myuser) {
  ...
  // https://0.0.0.0:8180/deliva/business/ericm
   ...
   //qInfo() << "My User handlenameresource is: " << QString::fromStdString(myuser) << Qt::endl;
...
if((request.urlParam("data")=="business")) {
  ...
  //qInfo() << "My Biz is: " << QString::fromStdString(mybiz) << Qt::endl;
...
std::string UserNameResource::fromIstream(std::istream &stream)
  ...
  ServerResource::~ServerResource()
   ...
   void ServerResource::handleRequest(const Wt::Http::Request &request, Wt::Http::Response &response) { // begin virtual void handleRequest
    ...
    std::string mybiz = request.urlParam("business");
    //qInfo() << "My Biz ServerResource is: " << QString::fromStdString(mybiz) << Qt::endl;
...
//     qInfo() << "SystemPrefix" << systemPrefix << Qt::endl;
//     QStringList args;

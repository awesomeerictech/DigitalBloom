...
{
    QRegExp regExp = QRegExp(filterRegularExpression().pattern());
    if (regExp.pattern() == filterPattern)
        return;

    regExp.setPattern(filterPattern);
    QSortFilterProxyModel::setFilterRegularExpression(regExp.pattern());
...
{
     QRegExp regExp = QRegExp(filterRegularExpression().pattern());
    return static_cast<PatternSyntax>(regExp.patternSyntax());
}

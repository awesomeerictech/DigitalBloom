# --- OpenSSL for Windows helper (put this inside your top-level CMakeLists) ---
if(WIN32)
  ...
  # 1) Prefer import libs in lib/ (libcrypto.lib, libssl.lib)
  if(EXISTS "${_lib_dir}")
  ...
  # 2) If no .lib import libs, try look for DLLs in bin/ (e.g. libcrypto-3-x64.dll)
  if((NOT _found_crypto_lib) OR (NOT _found_ssl_lib))
    if(EXISTS "${_bin_dir}")
  ...
  # 3) Set include dir if present
  if(EXISTS "${_include_dir}")
    set(_found_include "${_include_dir}")
  endif()

  # 4) Populate CACHE variables (prefer .lib import libs; if only DLLs found, use them as fallback)
  if(_found_crypto_lib)
    set(OPENSSL_CRYPTO_LIBRARY "${_found_crypto_lib}" CACHE STRING "OpenSSL crypto import lib (Windows)" FORCE)
  elseif(_found_crypto_dll)
    set(OPENSSL_CRYPTO_LIBRARY "${_found_crypto_dll}" CACHE STRING "OpenSSL crypto dll (Windows) - fallback" FORCE)
  endif()

  if(_found_ssl_lib)
    set(OPENSSL_SSL_LIBRARY "${_found_ssl_lib}" CACHE STRING "OpenSSL ssl import lib (Windows)" FORCE)
  elseif(_found_ssl_dll)
    set(OPENSSL_SSL_LIBRARY "${_found_ssl_dll}" CACHE STRING "OpenSSL ssl dll (Windows) - fallback" FORCE)
  endif()
  ...
  # you can append OPENSSL_SSL_LIBRARY if you want it listed as missing too
  ...
  # messages
  message(STATUS "OpenSSL (Windows) search:")
